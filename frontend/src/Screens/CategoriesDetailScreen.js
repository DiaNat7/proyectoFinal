...
export default function CategoriaDetailScreen({ route, navigation }) {
  // Guardamos la categoría en un estado local para que al editar, la pantalla se actualice sola
  ...
  const handleActualizar = async () => {
    ...
    try {
      ...
      // Actualizamos la vista al instante y cerramos el modal
      setCategoria(dataActualizada);
      setModalVisible(false);

      if (Platform.OS === 'web') window.alert("Categoría actualizada con éxito");
      else Alert.alert("¡Éxito!", "Categoría actualizada con éxito");
      
    } catch (error) {
      console.error(error);
      if (Platform.OS === 'web') window.alert("Error al actualizar: " + error.message);
  ...
  const ejecutarEliminacion = async () => {
    try {
      const response = await fetch(`${BASE_URL}/categorias/${categoria._id}`, { method: 'DELETE' });
  ...
  const handleEliminar = () => {
    if (Platform.OS === 'web') {
      const seguro = window.confirm("¿Segura que deseas eliminar esta categoría?");
      ...
      <ScrollView>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.buttonsContainer}>
            ...
            <TouchableOpacity style={styles.btnEditar} onPress={abrirModalEditar}>
...
const styles = StyleSheet.create({
  ...
  headerBanner: { width: '100%', height: 250, justifyContent: 'center', alignItems: 'center', elevation: 4 },
  ...
  title: { fontSize: 32, fontWeight: 'bold', color: '#333', marginBottom: 15, textAlign: 'center' },
  descText: { fontSize: 18, color: '#666', textAlign: 'center', marginBottom: 30, lineHeight: 26 },
