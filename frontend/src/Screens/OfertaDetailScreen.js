...
export default function OfertaDetailScreen({ route, navigation }) {
  ...
  // FUNCIÓN PARA AGREGAR A FAVORITOS CON DEBUG
  const handleAgregarFavorito = async () => {
    if (String(oferta._id).startsWith('ext-')) {
      Alert.alert("Aviso", "Esta es una oferta externa. Crea una nueva para probar.");
      return; 
    }
    ...
    try {
      ...
      // LOGS EXTREMOS PARA VER QUÉ PASA
      ...
      if (!response.ok) {
        const errorText = await response.text(); // Leemos el error crudo
  ...
  const handleActualizar = async () => {
    try {
      const token = await AsyncStorage.getItem("userToken");
      const datos = { titulo, descripcion, precioAnterior: Number(precioAnterior), precioActual: Number(precioActual), imagen };
    ...
    <View style={styles.container}>
      <ScrollView>
        <View style={styles.headerBanner}>
          <Image source={{ uri: oferta.imagen || "https://via.placeholder.com/400.png" }} style={styles.image} resizeMode="contain" />
        </View>
        
        <View style={styles.content}>
          <Text style={styles.title}>{oferta.titulo}</Text>
          <Text style={styles.descText}>{oferta.descripcion || "Sin descripción"}</Text>
          ...
          <View style={styles.priceSection}>
            ...
            <View style={styles.priceRowHighlight}>
              <Text style={styles.priceLabelHighlight}>Precio Ofertini:</Text>
          ...
          <View style={styles.buttonsContainer}>
            <TouchableOpacity style={styles.btnFavorito} onPress={handleAgregarFavorito}>
              <Text style={styles.btnFavoritoText}>❤️ Agregar a Favoritos</Text>
            </TouchableOpacity>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.rowInputs}>
              <View style={styles.halfInput}><TextInput style={styles.input} placeholder="Precio Ant." value={precioAnterior} onChangeText={setPrecioAnterior} keyboardType="numeric" /></View>
              <View style={styles.halfInput}><TextInput style={styles.input} placeholder="Precio Act." value={precioActual} onChangeText={setPrecioActual} keyboardType="numeric" /></View>
            </View>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={[styles.btnModal, styles.btnCancelar]} onPress={() => setModalVisible(false)}><Text style={styles.btnText}>Cancelar</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.btnModal, styles.btnGuardar]} onPress={handleActualizar}><Text style={styles.btnText}>Guardar</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 30, fontWeight: 'bold', color: '#333', marginBottom: 15, textAlign: 'center' },
  descText: { fontSize: 16, color: '#666', textAlign: 'center', marginBottom: 25, lineHeight: 24 },
  priceSection: { backgroundColor: '#FFF5F5', borderRadius: 15, padding: 20, marginBottom: 30, borderWidth: 1, borderColor: '#FFB6C1' },
  ...
  btnFavorito: { backgroundColor: '#FFB6C1', padding: 15, borderRadius: 10, alignItems: 'center' },
  ...
  modalContent: { backgroundColor: '#fff', borderRadius: 20, padding: 25, elevation: 5, width: '100%', maxWidth: 500, alignSelf: 'center' },
