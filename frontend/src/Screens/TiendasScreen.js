...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, ActivityIndicator, Image, TextInput, Modal, Platform, Alert } from "react-native";
...
export default function TiendasScreen({ navigation }) {
  ...
  const cargarTiendasBackend = async () => {
    try {
      ...
      if (Platform.OS !== 'web') Alert.alert("Error de conexión", "No se pudieron cargar las tiendas.");
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleGuardarTienda = async () => {
    if (!nombre || !ubicacion) {
      if (Platform.OS === 'web') window.alert("El nombre y la ubicación son obligatorios");
  ...
  return (
    <View style={styles.container}>
      ...
      {cargando ? (
        ...
        <View style={styles.loaderContainer}>
          <Text style={styles.loaderText}>No hay tiendas registradas. ¡Agrega la primera!</Text>
        </View>
      ) : (
        <FlatList
          data={tiendas}
      ...
      <Modal visible={modalVisible} animationType="fade" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} placeholder="Ej: Sucursal Centro" value={nombre} onChangeText={setNombre} />
            
            <Text style={styles.label}>Ubicación/Dirección:</Text>
            <TextInput style={styles.input} placeholder="Ej: Av. Principal 123" value={ubicacion} onChangeText={setUbicacion} />
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity style={[styles.btnModal, styles.btnGuardar]} onPress={handleGuardarTienda}>
...
const styles = StyleSheet.create({
  ...
  btnAgregar: { backgroundColor: "#E75480", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8 },
  ...
  cardContacto: { fontSize: 14, color: "#E75480", fontWeight: "600", marginBottom: 15 }, // <-- ESTILO PARA EL CONTACTO
  ...
  input: { borderWidth: 1, borderColor: '#FFB6C1', padding: 10, borderRadius: 10, marginBottom: 15, backgroundColor: '#FFF' },
